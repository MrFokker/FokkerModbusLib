...
class ModbusClientInterface
{
  public:
    ...
    virtual void WriteSingleCoil(uint8_t targetAddress, uint16_t registerAdress, bool value, const DoneCallback& callback) = 0;

    // Value example: 10 coils starting at coil 20,
    // Then coils order: 27 26 25 24 23 22 21 20 - - - - - - 29 28
    ...
    virtual void WriteSingleRegister(uint8_t targetAddress, uint16_t registerAdress, uint16_t value, const DoneCallback& callback) = 0;
    ...
    // Request: Function code (1 byte)
    // Response: Function code (1 byte), Byte Count (1 byte), Status (2 bytes), Event Count (2
    // bytes), Message Count (2 bytes), Events (variable)

    // 0x11 Report Server ID (Serial Line only)
    // Request: Function code (1 byte)
    // Response: Function code (1 byte), Byte Count (1 byte), Server ID (variable), Run Indicator
    // Status (1 byte), Additional Data (variable)
    ...
    // Request: Function code (1 byte), Byte Count (1 byte), FileRecordData, Data (N*2 bytes). See
    // chapter 6.15 Response: Function code (1 byte), Byte Count (1 byte), FileRecordData, Data (N*2
    // bytes). See chapter 6.15

    // 0x16 Mask Write Register
    // Request: Function code (1 byte), Reference Address (2 bytes), AND Mask (2 bytes), OR Mask (2
    // bytes) Response: Function code (1 byte), Reference Address (2 bytes), AND Mask (2 bytes), OR
    // Mask (2 bytes)

    // 0x17 Read/Write Multiple Registers
    // Request: Function code (1 byte), Read Start Address (2 bytes), Read Quantity (2 bytes), Write
    // Start Address (2 bytes), Write Quantity (2 bytes), Write Byte Count (1 byte), Write Data
    // (variable) Response: Function code (1 byte), Byte Count (1 byte), Read Data (variable)

    // 0x18 Read FIFO Queue
    // Request: Function code (1 byte), FIFO Pointer Address (2 bytes)
    // Response: Function code (1 byte), Byte Count (2 bytes), FIFO Count (2 bytes), FIFO Data (N ×
    // 2 bytes)
    ...
    // Response: Function code (1 byte), MEI Type = 0x0E (1 byte), Conformity Level, More Follows,
    // Next Object ID, Object List
};
