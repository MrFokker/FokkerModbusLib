...
private:
    ...
    void HandleTransmitComplete(ModbusPortInterface::PortResult result)
    {
        ...
        {
            m_port.Receive(m_buffer, m_timeoutMs, ModbusPortInterface::ReceiveCallback::create<ModbusClient, &ModbusClient::HandleReceiveComplete>(*this));
        }
        else
        {
            m_port.Cleanup();
    ...
    {
        // Validate Response
        uint8_t pduSize = Framer::ValidateAduFrame(m_currentRequest.m_targetAddress, m_buffer.data(), length);
    ...
    bool ParsePduFrame(uint8_t pduOffset, uint8_t pduSize)
    {
        ...
        else if ((m_currentRequest.m_functionCode == ModbusDefinitions::FunctionCodes::WRITE_MULTIPLE_COILS) || (m_currentRequest.m_functionCode == ModbusDefinitions::FunctionCodes::WRITE_MULTIPLE_REGISTERS) ||
                 (m_currentRequest.m_functionCode == ModbusDefinitions::FunctionCodes::WRITE_SINGLE_COIL) || (m_currentRequest.m_functionCode == ModbusDefinitions::FunctionCodes::WRITE_SINGLE_REGISTER))
        {
